...
class TaskList {
  ...
  validationForm(title, description) {
    const titleRegex =
      /^(?!\s)(?![\d\s]+$)([a-zA-Z]{1,16}|[а-яА-ЯёЁ]{1,16}|\d{1,16})(\s([a-zA-Z]{1,16}|[а-яА-ЯёЁ]{1,16}|\d{1,16})){1,15}(?<!\s)$/u;
    const descriptionRegex =
      /^(?!\s+$)([a-zA-Z\s]{1,16}|[а-яА-ЯёЁ\s]{1,16}|\d{1,16})(\s([a-zA-Z\s]{1,16}|[а-яА-ЯёЁ\s]{1,16}|\d{1,16})){0,15}\s*$/u;
    return (
  ...
  addTasks(...tasks) {
    tasks.forEach((task) => {
      const taskExists = this.#tasks.find(
        (existingTask) => existingTask.getId() === task.getId()
      );
      if (!taskExists) {
...
taskList.addEventListener("click", (e) => {
  ...
  if (link) {
    ...
    window.location.href = `src/page/details.html?id=${encodeURIComponent(
      taskId
    )}`;
  }
});
